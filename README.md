...
- For simplicity, I just had three flask servers spun up programatically from `test/run_query_servers.py`, and they are just loading data from JSON files. In a real-world scenario, I'd be using databases instead to store the patient data, but this test is just to simulate what an actual server might look like, and I don't want to reduce portability.
- I decided not to set up a linter, but I definitely would use something like `pylint`, `mypy`, `pycodestyle`, etc. in a serious project.